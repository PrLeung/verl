...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        # 从 config 读取 batchsize 和 rollout 数量（带有稳健的字段回退）
        cfg = self.config
        batchsize = (
        ...
        self.sampling_params = SamplingParams(
            ...
            **kwargs) if self.answer_suffix_mode == "stage2" or self.answer_suffix_mode == "stage1_1" else SamplingParams(**kwargs)
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "multi_modal_data" in non_tensor_batch:
            if hasattr(self, 'logits_processor') and self.logits_processor is not None:
                data_sources = []
                for multi_modal_data in non_tensor_batch["multi_modal_data"]:
                    data_source = multi_modal_data.pop("data_source", None)
                    data_sources.append(data_source)
                
                if data_sources:
                    self.logits_processor.set_data_sources(data_sources)
            vllm_inputs = []
