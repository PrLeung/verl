...
def format_reward_think(predict_str: str) -> float:
    ...
    # Anchor: any content then </think> then <answer>...</answer> till the end
    pattern = re.compile(r"^[\s\S]*?</\|think\|>\s*<\|answer\|>[\s\S]*?</\|answer\|\>\s*$", re.IGNORECASE)
...
#     pattern = re.compile(r"^\s*</no_think>\s*<answer>[\s\S]*?</answer>\s*$", re.IGNORECASE)
#     anchored_match = pattern.search(predict_str) is not None
...
#     open_no_think_count = len(re.findall(r"(?i)<no_think>", predict_str))
#     close_no_think_count = len(re.findall(r"(?i)</no_think>", predict_str))
#     open_answer_count = len(re.findall(r"(?i)<answer>", predict_str))
#     close_answer_count = len(re.findall(r"(?i)</answer>", predict_str))
...
def format_reward_no_think(predict_str: str) -> float:
    ...
    # Anchor: 必须严格匹配 </no_think><answer>...</answer>
    ...
    # -----------------------------
    # 修改部分：</no_think> 前的 token 数
    # -----------------------------
    before_no_think = re.split(r"(?i)</\|think_no\|>", predict_str)[0].strip()
    word_count = len(before_no_think.split()) if before_no_think else 0

    base_score = 0  # 出现 </no_think> 的基础分
    if word_count > 10:
        penalty = 0.0
    else:
        penalty = (1.0 - base_score) / (1.0 + word_count)
...
def compute_score(data_source, solution_str, ground_truth, format_score: float = 0.5, extra_info=None) -> float:
