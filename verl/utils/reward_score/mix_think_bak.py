...
def format_reward_no_think(predict_str: str, stage: str = "stage2") -> float:
    if not predict_str:
        return 0.0

    # Anchor: 必须匹配 <no_think>...</no_think> 然后 <answer>...</answer>，并覆盖整串
    pattern = re.compile(r"^<\|think_no\|>[\s\S]*?</\|think_no\|>\s*<\|answer\|>[\s\S]*?</\|answer\|>\s*$", re.IGNORECASE)
    anchored_match = pattern.search(predict_str) is not None
    ...
    # 提取<|think_no|>和</|think_no|>中间的内容
    think_no_pattern = re.compile(r"<\|think_no\|>\s*(.*?)\s*</\|think_no\|>", re.DOTALL | re.IGNORECASE)
    think_no_match = think_no_pattern.search(predict_str)
    

    if think_no_match:
        think_no_content = think_no_match.group(1).strip()
        word_count = len(think_no_content.split()) if think_no_content else 0
        if word_count > 20:
            return 0.0
        elif word_count == 0:
            return 1.0
            ...
            # 1到20之间指数递减：从0.5递减到接近0
            # 使用指数函数：0.5 * exp(-k * (word_count - 1))
            # 当word_count=1时，值为0.5
            # 当word_count=20时，值接近0
            k = 0.3  # 调整衰减速度
            return 0.5 * math.exp(-k * (word_count - 1))
    else:
        return 0.0



def format_reward(predict_str: str, data_source: str = None, stage: str = "stage2") -> float:
    ...
    if data_source == "think" or data_source == "llava_cot":
        ...
        return format_reward_no_think(predict_str, stage)
    else:
        return 0.0
